...
- 252 independent exercises (42 existing foundations + 168 questions + 42 new varied tasks), 14 German grammar chapters, and explicit basic/application/transfer levels.
...
The 168 additional questions live in `content/new-exercises.json`, with supplemental rules in `content/grammar-additions.json`. Run `python3 content/build-training.py` to regenerate `dist/training-data.js`. Existing IDs and original test data stay unchanged. Every topic has 18 independent questions: 7 foundation, 5 application and 6 transfer. The 42 varied tasks are authored in `content/varied-exercises.json`. Practice offers free input, native answer choices, click/tap sentence assembly and identification of one incorrect sentence segment, followed by the corrected sentence. Each topic includes all four forms. Existing suitable selection prompts are rendered as radio choices without changing their IDs or answer keys. Topic rounds contain up to 12 questions; mixed rounds contain 12 distinct topics. Selection prioritises unseen questions, then unresolved mistakes, then known questions; it alternates available exercise formats within each priority group. Students can also select an exercise format directly. Incomplete choices and sentence puzzles persist locally, including through rule lookup and reload. No claim of unlimited generated exercises is made.

Run `node tests/verify.cjs` with jsdom available to check grading, controls, local persistence and the five complete original exams.
